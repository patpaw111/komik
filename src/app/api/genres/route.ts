...
export async function GET() {
  try {
    ...
    if (error) {
      ...
      return fail("Gagal mengambil genres", 500, error.message);
    }

    return ok(data ?? []);
  } catch (err: any) {
    console.error("[GET /api/genres] unexpected error", err);
    return fail("Terjadi kesalahan saat mengambil genres", 500);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return fail("Gagal membuat genre", 500, error.message);
    }

    return NextResponse.json({ success: true, data }, { status: 201 });
  } catch (err: any) {
    console.error("[POST /api/genres] unexpected error", err);
    return fail("Terjadi kesalahan saat membuat genre", 500);
  }
}
