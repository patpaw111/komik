...
export function GenreTable({ items, error }: Props) {
  const router = useRouter();
  const [search, setSearch] = useState("");
  const [editing, setEditing] = useState<GenreItem | null>(null);
  ...
  const startEdit = (item: GenreItem) => {
    setEditing(item);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        setActionError(json.message ?? "Gagal mengubah genre");
        return;
      }
      ...
      console.error("[GenreTable] edit error", err);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        setActionError(json.message ?? "Gagal menghapus genre");
        return;
      }

      router.refresh();
    } catch (err) {
      console.error("[GenreTable] delete error", err);
      setActionError("Terjadi kesalahan saat menghapus genre");
    } finally {
      setDeletingId(null);
    }
  };
    ...
    <div className="space-y-3">
      ...
      {error && (
        <p className="text-sm text-red-400">
          Gagal mengambil data genres. Coba refresh halaman.
        </p>
      )}

      {filteredItems.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          Belum ada genre. Tambahkan genre baru dari form di sebelah kanan.
        </p>
      ) : (
      ...
      {editing && (
        <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-sm rounded-lg border border-border bg-card p-4 text-sm shadow-lg">
            <h4 className="mb-2 font-medium">Edit Genre</h4>
