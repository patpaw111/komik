...
export function AuthorTable({ items, error }: Props) {
  ...
  const startEdit = (item: AuthorItem) => {
    setEditing(item);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        const errorMsg = json.message ?? "Gagal mengubah author";
        let errorDetails = "";
        if (json.details) {
      ...
      console.error("[AuthorTable] edit error", err);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        setActionError(json.message ?? "Gagal menghapus author");
        return;
      }

      router.refresh();
    } catch (err) {
      console.error("[AuthorTable] delete error", err);
      setActionError("Terjadi kesalahan saat menghapus author");
    } finally {
      setDeletingId(null);
    }
  };
    ...
    <div className="space-y-3">
      ...
      {error && (
        <p className="text-sm text-red-400">
          Gagal mengambil data authors. Coba refresh halaman.
        </p>
      )}

      {filteredItems.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          Belum ada author. Tambahkan author baru dari form di sebelah kanan.
        </p>
      ) : (
      ...
      {editing && (
        <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-sm rounded-lg border border-border bg-card p-4 text-sm shadow-lg">
            <h4 className="mb-2 font-medium">Edit Author</h4>
